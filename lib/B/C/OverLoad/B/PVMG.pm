...
use B::C::File qw/init init1 init2 initPV svsect xpvmgsect xpvsect pmopsect/;
...
sub save {
    ...
    if ( defined($pv) and !$static ) {
        ...
        if ($shared_hek) {
            ...
            initPV()->add( sprintf( "sv_list[%d].sv_u.svu_pv = %s->shared_he_hek.hek_key; /* PVMG */", $sv_ix, $hek ) )
              unless $hek eq 'NULL';
        }
    }
